...
public class OutputHandler {
	...
	private static void exportImg(String imgType, OutputStream ostream, Collection<GridElement> entities, FontHandler diagramFont) throws IOException {
		int scale = Config.getInstance().getExportScale();
		// At first try to export using correct DPI settings (for high dpi/retina displays) see https://stackoverflow.com/questions/321736/how-to-set-dpi-information-in-an-image/4833697
		...
		if (!exported) { // if the export with correct DPI fails, use the simple one (e.g. BMP doesnt seem to work with correct DPI settings)
	...
	private static void setImgDPI(IIOMetadata metadata) throws IIOInvalidTreeException {
		final int DPI = 72 * 2;
